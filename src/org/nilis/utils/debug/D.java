...
import org.nilis.utils.device_data.DeviceUtils;
import org.nilis.utils.other.TimeUtils;
import org.nilis.utils.remote_interaction.StringPoster;
...
public class D {
	public static final String SERVER_URL = "http://remote-log-tool.appspot.com";
	private static final String LOG_URL = SERVER_URL + "/api?op=put_message";
	
	private static final String LOG_FOLDER_NAME = "/logs";
	private static final String LOG_FILENAME_PATTERN = "yyyyMMdd_HH_mm_ss";
	...
	private static String deviceName = DeviceUtils.getDeviceName();
	...
	public static void enableRemoteLogging(final Context contextToUse) {
		...
		try {
			D.deviceId = DeviceUtils.getDeviceIMEI(contextToUse);
	...
	public static void logToFile(final Throwable throwable) {
		if(!debug || throwable==null) {
			return;
		}
		
		File dir = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + LOG_FOLDER_NAME);
		dir.mkdirs();
		
		Calendar c = Calendar.getInstance();
		String fileName = DateUtils.formatDate(c.getTime(), LOG_FILENAME_PATTERN);
